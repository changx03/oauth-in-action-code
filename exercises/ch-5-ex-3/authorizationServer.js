...
app.get('/authorize', function (req, res) {
  ...
  if (!client) {
     ...
     */
    const resScope = req.query.scope ? req.query.scope.split(' ') : null
    const clientScope = client.scope ? client.scope.split(' ') : null
    // don't have the right
    if (__.difference(resScope, clientScope).length > 0) {
     ...
     */
    res.render('approve', { client: client, reqid: reqid, scope: resScope })
  }
})
...
app.post('/approve', function (req, res) {
  ...
  if (req.body.approve) {
    if (query.response_type == 'code') {
       ...
       */
      const resScope = getScopesFromForm(req.body)
      ...
      codes[code] = { request: query, scope: resScope }
        ...
        if (!value || value['client_id'] !== clientId) {
          nosql.remove().make(function (builder) {
            ...
            builder.callback(function (err, count) {
              ...
              console.log('remove %d with client_id: %s', count, clientId)
              res.status(400).json({ error: 'invalid_grant' })
            })
          })
        }
