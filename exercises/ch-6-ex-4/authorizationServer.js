    ...
    nosql.one().make(function (builder) {
      ...
      builder.callback(function (err, value) {
        ...
        if (!value) {
          ...
          if (value['client_id'] != clientId) {
            nosql.remove().make(function (builder) {
              ...
              builder.callback(function (err, count) {
                ...
                console.log('Removed %d rows which contain refresh_token %s', count, value['refresh_token'])
                res.status(400).json({ error: 'invalid_grant' })
                return
              })
            })
          ...
          // nosql.insert({ refresh_token: refreshToken, client_id: clientId, scope: code.scope })
          const tokenResponse = {
